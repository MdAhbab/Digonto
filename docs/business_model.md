...
2. **Verified-consultancy certification.** Consultancies that join the sector's voluntary association have a market interest in separating themselves from the majority that operate on a general trade licence with no specialised supervision. Digonto offers a certification listing tied to published conduct standards and student reviews. This converts the project's main adversary into a customer while raising sector standards.
...
The design starts from real usage constraints in Bangladesh, not from a persona template. Bangla is the first language of the interface, with Banglish input accepted. Voice input for users more comfortable speaking than typing is planned, using the model's own audio capability, but is not built yet: no speech-to-text service is deployed, so the interview room accepts typed answers only today. Connections in Bangladesh are often slow rather than absent, so the design target is a usable first paint on a mid-range Android over a congested mobile network: first load is budgeted under 300 KB of compressed JavaScript, fonts are self-hosted and subset so no third-party request sits on the critical path, and every answer streams token by token so the student sees progress rather than a spinner. Literacy range is respected through plain-language explanation patterns (every technical term explained at first use, in Bangla). The pilot with students from at least three districts outside Dhaka exists to test these assumptions against reality, and pilot feedback flows into the replay buffer, so lived experience literally trains the system.
